...
class DeepQNetwork:
    ...
    def _build_net(self):
        ...
        self.eval_network = Sequential([
            ...
            layers.Dense(nn_count, activation=tf.nn.relu),
            layers.Dense(self.n_actions,activation=tf.nn.sigmoid)
        ])
    ...
    def learn(self):
        ...
        batch_state = batch_memory[:, self.n_features-1]
        batch_state_ = batch_memory[:, -self.n_features]
        ...
        with tf.GradientTape() as tape:
            ...
            update = tf.stop_gradient(reward + self.gamma*tf.reduce_max(q_next,axis=1))
